...
> **目標**：通過分析用戶與 AI 的對話紀錄，智能識別社交困擾模式，提供個人化的心理支持與成長建議。

#### 核心特色

1. **🔍 智能模式識別**
   - 自動偵測重複的社交困擾主題（如：人際焦慮、溝通障礙、情緒管理）
   - 追蹤情緒變化趨勢，建立個人情緒地圖
   - 識別觸發負面情緒的特定情境或對話模式

2. **📈 進度追蹤儀表板**
   - 視覺化呈現社交健康狀態（情緒趨勢圖、關鍵字詞雲）
   - 對比過去報告，量化個人成長進度
   - 顯示已改善的困擾與仍需關注的領域

3. **💡 個人化建議引擎**
   - 基於對話分析，提供具體可執行的改善策略
   - 由 Tororo（情緒療癒助手）生成溫暖、同理心的鼓勵話語
   - 推薦相關的學習資源或練習活動

4. **🔔 智能提醒系統**
   - 偵測到情緒低潮時主動關懷
   - 定期生成社交健康報告（週報/月報）
   - 當用戶在對話中提到困擾時，智能推薦分析功能
...
Chief Agent 觸發分析
    ↓ (分發任務)
Sub-Agent (Tororo) 深度分析
    ↓
生成 SocialAnalysisReport
    ↓
前端視覺化呈現
```

**新增資料模型**：
- `SocialAnalysisReport` - 儲存分析報告
- `EmotionTrack` - 記錄情緒變化時間線
- `GrowthMilestone` - 追蹤個人成長里程碑

**新增後端服務**：
- `conversationAnalysisService.ts` - 對話分析引擎
- `emotionTrackingService.ts` - 情緒追蹤服務
- `socialInsightService.ts` - 社交洞察生成器

**前端新增頁面**：
- `/analysis/social` - 社交健康分析儀表板
- `/analysis/emotion-timeline` - 情緒時間線視圖
- `/analysis/growth-report` - 成長報告頁面
...
1. **主動分析**
   ```
   用戶："幫我分析一下我最近的社交困擾"
   系統：觸發 Chief Agent → 收集對話 → Tororo 分析 → 生成報告
   ```

2. **智能推薦**
   ```
   系統偵測：用戶連續 3 天提到「不敢跟同事說話」
   觸發：「我注意到你最近在社交方面有些困擾，要不要我幫你分析一下？」
   ```

3. **定期報告**
   ```
   每週日晚上：自動生成本週社交健康報告
   包含：情緒統計、主要困擾、進步亮點、下週建議
   ```

#### 隱私與倫理

- **數據安全**：所有分析結果僅用戶可見，不對外分享
- **用戶控制**：可隨時關閉分析功能，刪除歷史報告
- **透明化**：清楚說明 AI 如何分析數據，以及分析依據
- **非醫療性質**：明確標註此功能為輔助工具，嚴重心理問題建議尋求專業協助

#### 開發優先級

- **Phase 1**（MVP）：基礎對話分析 + 簡單情緒追蹤
- **Phase 2**：進階模式識別 + 個人化建議引擎
- **Phase 3**：視覺化儀表板 + 進度追蹤系統
- **Phase 4**：智能提醒 + 多模態分析（表情、語音）
