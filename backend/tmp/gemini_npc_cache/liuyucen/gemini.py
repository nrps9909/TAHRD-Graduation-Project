#!/usr/bin/env python3
"""
NPC liuyucen 專用的 Gemini 執行腳本 - 完整優化版
...
class OptimizedGeminiClient:
    ...
    def __init__(self):
        ...
        self.npc_name = "liuyucen"
        self.npc_display_name = "劉宇岑"
        ...
        if Path("/tmp").exists() and os.access("/tmp", os.W_OK):
            self.work_dir = Path("/tmp") / "gemini_liuyucen_cache"
        else:
    ...
    def _prepare_single_context(self):
        ...
        try:
            ...
            # 1. 人設檔案（注意檔名拼寫）
            personality_file = self.current_dir / "LiuYuCen_Personality.md"
            ...
            # 2. 對話風格（注意檔名拼寫）
            chat_style_file = self.current_dir / "LiuYuCen_Chat_style.txt"
