...
**特殊格式：如果內容是「聊天記錄」或「社交互動記錄」，請使用以下結構化格式進行分析：**

[主題]：對話/事件名稱（例：與朋友起衝突、主動打招呼練習）
[情境]：簡述當下發生什麼（限一句話）
[使用者反應]：描述使用者的情緒或行為反應
[AI 回饋]：AI 的建議或安撫（具體一句話）
[社交能力標籤]：#表達情緒 #傾聽 #自我覺察 #主動互動（可多選）
[進度變化]：成長 +1／維持 0／退步 -1（用來追蹤趨勢）
[建議行動]：AI 給下一步具體任務

當內容是聊天記錄時，請在 JSON 的 "analysis" 欄位中包含上述格式化的分析。
