...
const IntroScreen = ({ triggerFeedback }: IntroScreenProps) => {
  const navigate = useNavigate();
  const { startGame, setOS } = useGameStore();
  const [userCount, setUserCount] = useState(0);
  const [loading, setLoading] = useState(false);
  const live2dSize = useLive2DSize('intro');
  ...
  const handleLogin = async () => {
    ...
    const response = await apiService.login(introState.email, introState.password);
  ...
  const handleRegister = async () => {
    if (!introState.playerName.trim() || !introState.email.trim() || !introState.password.trim()) {
          ...
          <div className="w-full h-full grid lg:grid-cols-[38.2fr_61.8fr] grid-cols-1 lg:grid-rows-1 grid-rows-[auto_1fr] gap-0">
              ...
              >
                ...
                <div className="text-center mb-4 sm:mb-6 lg:mb-8 w-full max-w-sm">
                  ...
                  >
                    <span className="text-2xl sm:text-3xl lg:text-3xl xl:text-4xl">🚀</span>
                  </motion.div>
                ...
                <div className="w-full max-w-sm lg:max-w-md space-y-3 sm:space-y-4 lg:space-y-5">
                  ...
                  <div className="flex p-1 bg-white/70 backdrop-blur rounded-2xl shadow-lg border border-pink-200/40">
                      <button
                        onClick={() => setIntroState(prev => ({ ...prev, authMode: 'login', error: '' }))}
                      ...
                      <button
                        onClick={() => setIntroState(prev => ({ ...prev, authMode: 'register', error: '' }))}
                        className={`flex-1 py-2 sm:py-2.5 lg:py-3 rounded-xl font-semibold transition-all text-sm lg:text-base ${
                    ...
                    <div className="space-y-3 sm:space-y-4">
                      <AnimatePresence mode="wait">
                        {introState.authMode === 'register' && (
                          ...
                          >
                            <label className="block text-gray-700 text-sm font-medium mb-2">暱稱</label>
                            <div className="relative">
                              <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                              <input
                                type="text"
                                value={introState.playerName}
                                onChange={(e) => setIntroState(prev => ({ ...prev, playerName: e.target.value }))}
                                placeholder="輸入你的暱稱"
                      ...
                      <div>
                        <label className="block text-gray-700 text-sm font-medium mb-2">電子郵件</label>
                        <div className="relative">
                          <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                          <input
                            type="email"
                            value={introState.email}
                            onChange={(e) => setIntroState(prev => ({ ...prev, email: e.target.value }))}
                      ...
                      <div>
                        <label className="block text-gray-700 text-sm font-medium mb-2">密碼</label>
                        <div className="relative">
                          <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                          <input
                            type="password"
                            value={introState.password}
                            onChange={(e) => setIntroState(prev => ({ ...prev, password: e.target.value }))}
                      ...
                      <motion.button
                        ...
                        disabled={loading ||
                          ...
                          (introState.authMode === 'register' && !introState.playerName.trim())
                        }
                      ...
                      >
                        {loading
                          ? <span className="flex items-center justify-center"><span className="animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2"></span>處理中...</span>
                          : (introState.authMode === 'login' ? '登入' : '建立帳號')
                        }
                      </motion.button>
                    </div>
                    ...
                    <div className="mt-3 sm:mt-4 lg:mt-5 text-center">
                      <p className="text-xs sm:text-xs lg:text-sm text-gray-500">
                        {introState.authMode === 'login' ? (
                          <>還沒有帳號？ <button onClick={() => setIntroState(prev => ({ ...prev, authMode: 'register' }))} className="text-pink-500 hover:text-pink-600 font-medium">立即註冊</button></>
                        ) : (
                          <>已經有帳號？ <button onClick={() => setIntroState(prev => ({ ...prev, authMode: 'login' }))} className="text-pink-500 hover:text-pink-600 font-medium">立即登入</button></>
                        )}
                      </p>
                ...
                <div className="h-full flex flex-col items-center justify-center px-6 lg:px-8 xl:px-12 py-8">
                  ...
                  >
                    ...
                    <div className="flex items-center justify-center gap-2 lg:gap-3 mb-3 lg:mb-4">
                      ...
                      <span className="text-gray-700 text-lg lg:text-xl xl:text-2xl font-medium">學會</span>
                      ...
                      <div className="relative w-[200px] h-[200px] lg:w-[280px] lg:h-[280px] xl:w-[340px] xl:h-[340px] 2xl:w-[400px] 2xl:h-[400px]">
                        ...
                        <Suspense fallback={
                          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-pink-100 to-yellow-100 rounded-full shadow-2xl">
                            <div className="text-pink-600 text-lg animate-pulse">🐱 載入中...</div>
                          </div>
                        }>
                          <Live2DPixi6
                            modelPath={APP_CONFIG.live2d.models.tororo.white}
                            fallbackImage={APP_CONFIG.live2d.models.tororo.fallback}
                            width={400}
                            height={400}
                            scale={0.20}
                            triggerMotion={true}
                    ...
                    <div className="absolute inset-0">
                      ...
                      >
                        <div className="bg-white/90 backdrop-blur-xl rounded-2xl lg:rounded-3xl p-4 lg:p-6 shadow-xl border border-pink-200/60 hover:scale-110 transition-all duration-300 hover:shadow-2xl cursor-pointer group">
                          <div className="text-center">
                            ...
                            <h3 className="text-gray-800 font-bold text-sm lg:text-base">可愛陪伴</h3>
                            <p className="text-gray-600 text-xs lg:text-sm mt-1">超萌老師</p>
                          </div>
                        </div>
                      </motion.div>
                      ...
                      >
                        <div className="bg-white/90 backdrop-blur-xl rounded-2xl lg:rounded-3xl p-4 lg:p-6 shadow-xl border border-yellow-200/60 hover:scale-110 transition-all duration-300 hover:shadow-2xl cursor-pointer group">
                          <div className="text-center">
                            <div className="text-3xl lg:text-4xl mb-2 group-hover:scale-110 transition-transform">⚡</div>
                            <h3 className="text-gray-800 font-bold text-sm lg:text-base">極速上手</h3>
                            <p className="text-gray-600 text-xs lg:text-sm mt-1">3分鐘入門</p>
                          </div>
                        </div>
                      </motion.div>
                      ...
                      >
                        <div className="bg-white/90 backdrop-blur-xl rounded-2xl lg:rounded-3xl p-4 lg:p-6 shadow-xl border border-pink-300/60 hover:scale-110 transition-all duration-300 hover:shadow-2xl cursor-pointer group">
                          <div className="text-center">
                            <div className="text-3xl lg:text-4xl mb-2 group-hover:scale-110 transition-transform">🤖</div>
                            <h3 className="text-gray-800 font-bold text-sm lg:text-base">AI助力</h3>
                            <p className="text-gray-600 text-xs lg:text-sm mt-1">智能輔助</p>
                          </div>
                        </div>
                      </motion.div>
                      ...
                      >
                        <div className="bg-white/90 backdrop-blur-xl rounded-2xl lg:rounded-3xl p-4 lg:p-6 shadow-xl border border-yellow-300/60 hover:scale-110 transition-all duration-300 hover:shadow-2xl cursor-pointer group">
                          <div className="text-center">
                            <div className="text-3xl lg:text-4xl mb-2 group-hover:scale-110 transition-transform">🎯</div>
                            <h3 className="text-gray-800 font-bold text-sm lg:text-base">零基礎</h3>
                            <p className="text-gray-600 text-xs lg:text-sm mt-1">輕鬆開始</p>
                          </div>
                        </div>
                      </motion.div>
                    </div>
