...
const InteractiveLesson = ({ sceneId = 'default' }: InteractiveLessonProps = {}) => {
  const { unlockAchievement, setWaitingForAI, setAIResponseReceived } = useGameStore();
  ...
  const [lessonState, setLessonState] = usePageStatePersistence(persistenceKey, {
    currentDialogueId: 1,
    lessonProgress: 0,
  ...
  const [currentDialogue, setCurrentDialogue] = useState(() => {
    const dialogue = lessonDialogues.find(d => d.id === lessonState.currentDialogueId);
  ...
  useEffect(() => {
    ...
    if (dialogue) {
      ...
      if (dialogue.action === 'celebrate' && !lessonState.completedDialogues.includes(dialogue.id)) {
      ...
      console.warn(`Dialogue with id ${lessonState.currentDialogueId} not found, resetting to first`);
      setLessonState(prev => ({
        ...prev,
        currentDialogueId: 1,
        lessonProgress: 0
    ...
    // Update progress
    const progress = (lessonState.currentDialogueId / lessonDialogues.length) * 100;
    if (Math.abs(progress - lessonState.lessonProgress) > 0.1) {
      setLessonState(prev => ({ ...prev, lessonProgress: progress }));
    }
  }, [lessonState.currentDialogueId, lessonState.completedDialogues, unlockAchievement, setLessonState]);
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center p-8 relative overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-10 pointer-events-none">
          ...
          <div className="absolute top-1/2 left-20 text-6xl animate-ping">💕</div>
          <div className="absolute top-20 right-20 text-7xl">🌸</div>
          <div className="absolute bottom-20 left-1/3 text-6xl animate-pulse">🎀</div>
        </div>
        ...
        <div className="absolute top-4 right-4 bg-white/90 backdrop-blur text-cat-dark px-4 py-2 rounded-full text-sm font-bold shadow-lg border border-cat-pink/30 flex items-center gap-2">
          <span>📖</span>
          <span className="font-chinese">課程 {lessonState.currentDialogueId} / {lessonDialogues.length}</span>
          <span>🐈</span>
        </div>
      </div>
    </div>
