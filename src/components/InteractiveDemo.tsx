  ...
  return (
    <div className="space-y-6">
      <div className="terminal-window">
        <h3 className="text-retro-amber font-pixel text-sm mb-4">INTERACTIVE DEMO</h3>
        <div className="space-y-3">
          {demo.steps.map((step, index) => (
            <motion.div
              ...
              initial={{ opacity: 0.3 }}
              animate={{
                opacity: completedSteps[index] ? 1 : currentStep === index ? 0.8 : 0.3,
              ...
              className={`p-3 border rounded ${
                completedSteps[index]
                  ...
                  : currentStep === index
                  ? 'border-retro-amber bg-amber-900 bg-opacity-10'
                  : 'border-gray-700'
              }`}
            >
              <div className="flex items-center gap-3">
                <span className="text-retro-green font-mono">{index + 1}.</span>
                <span className="flex-1 text-terminal-text font-mono text-sm">{step}</span>
                {completedSteps[index] && (
      ...
      <div className="terminal-window">
        <h3 className="text-retro-cyan font-pixel text-sm mb-4">PROGRESS</h3>
        <div className="flex gap-2 mb-4">
          {demo.steps.map((_, index) => (
            <div
              key={index}
              className={`h-2 flex-1 ${
                completedSteps[index] ? 'bg-green-500' : 'bg-gray-700'
              }`}
            />
          ))}
        </div>
