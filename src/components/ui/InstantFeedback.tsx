...
export const useInstantFeedback = () => {
  ...
  const showFeedback = (type: FeedbackType, message: string, options?: {
    value?: number;
    position?: { x: number; y: number };
  }) => {
  ...
  const showProgress = (message: string, position?: { x: number; y: number }) => {
    showFeedback('progress', message, { position });
  };

  const showSkill = (skillName: string, position?: { x: number; y: number }) => {
    showFeedback('skill', `學會了 ${skillName}`, { position });
  };

  const showEncouragement = (message: string, position?: { x: number; y: number }) => {
    showFeedback('encouragement', message, { position });
  };

  const showMilestone = (message: string, position?: { x: number; y: number }) => {
  ...
  const showPerfect = (message: string = '完美！', position?: { x: number; y: number }) => {
