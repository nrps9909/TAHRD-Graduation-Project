...
const AchievementNotification: React.FC<AchievementNotificationProps> = ({
  ...
  useEffect(() => {
    if (show) {
      ...
      confetti({
        particleCount: achievement.rarity === 'legendary' ? 150 :
                      achievement.rarity === 'epic' ? 100 :
                      achievement.rarity === 'rare' ? 75 : 50,
        spread: 70,
        origin: { y: 0.6 },
    ...
    <AnimatePresence>
      {show && (
        <div className="fixed inset-0 flex items-center justify-center z-50 pointer-events-none">
          ...
          >
            <div className={`bg-gradient-to-r ${getRarityStyles()} p-1 rounded-2xl shadow-2xl max-w-md mx-4`}>
              <div className="bg-gray-900 rounded-xl p-6 text-center relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  >
                    {achievement.icon === '🏆' ? <Trophy className="w-16 h-16 text-yellow-400 mx-auto" /> :
                     achievement.icon === '⭐' ? <Star className="w-16 h-16 text-yellow-400 mx-auto" /> :
                     achievement.icon === '⚡' ? <Zap className="w-16 h-16 text-blue-400 mx-auto" /> :
                     achievement.icon === '🎁' ? <Gift className="w-16 h-16 text-purple-400 mx-auto" /> :
                  ...
                  >
                    <div className={`inline-block px-3 py-1 rounded-full text-sm font-bold mb-2 ${
                      achievement.rarity === 'legendary' ? 'bg-gradient-to-r from-pink-500 to-yellow-500 text-white' :
                      achievement.rarity === 'epic' ? 'bg-gradient-to-r from-orange-500 to-red-500 text-white' :
                      achievement.rarity === 'rare' ? 'bg-gradient-to-r from-blue-500 to-purple-500 text-white' :
