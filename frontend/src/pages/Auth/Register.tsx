...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.username) newErrors.push('請輸入使用者名稱')
    else if (formData.username.length < 3) newErrors.push('使用者名稱至少需要 3 個字元')

    if (!formData.email) newErrors.push('請輸入電子郵件')
    else if (!formData.email.includes('@')) newErrors.push('請輸入有效的電子郵件')

    if (!formData.password) newErrors.push('請輸入密碼')
    else if (formData.password.length < 6) newErrors.push('密碼至少需要 6 個字元')
      ...
      >
        <div className="w-full max-w-md">
          ...
          >
            ...
            <p className="text-gray-700 font-medium">開始你的療癒之旅</p>
          </motion.div>
