-- Seed NPCs with specific IDs matching frontend
-- This script should be run after the database is initialized

-- First, check if NPCs already exist
DO $$
BEGIN
    -- Only insert if no NPCs exist
    IF NOT EXISTS (SELECT 1 FROM npcs LIMIT 1) THEN
        ...
        RAISE NOTICE 'NPCs inserted successfully';
    ELSE
        RAISE NOTICE 'NPCs already exist, skipping insertion';
    END IF;
END $$;